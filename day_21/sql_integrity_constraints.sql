...
create  table  company(id INT Primary key, name varchar(20) UNIQUE, location varchar(20) NOT NULL, capital_amount decimal check(capital_amount>50000), established_date date NOT NULL)ENGINE=InnoDB;
...
create table employees(emp_id int primary key, emp_name varchar(20) UNIQUE,emp_salary decimal NOT NULL, emp_role varchar(20), 
							emp_company_id  int, FOREIGN KEY(emp_company_id)  references company(id) on delete cascade on update cascade)ENGINE=InnoDB;
...
delete from company where id=1002;

drop table employees;
select *from employees;
select *from company;

update company set id=2222 where id=1002; -- To update company id with new id
